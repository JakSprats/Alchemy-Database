...
#ifdef ALCHEMY_DATABASE
        sds ovrd_key; unsigned char err = 0;
        keyindex = DXDB_getKeysFromCommand(mcmd, margv, margc, &numkeys,
                                          REDIS_GETKEYS_ALL, &ovrd_key, &err);
        //printf("err: %d ovrdkey: %s keyindex: %p\n", err, ovrd_key, keyindex);
        if (err) return NULL;
        if (ovrd_key) { // copy of lines 1742-1744
            slot = keyHashSlot(ovrd_key, sdslen(ovrd_key));
            n    = server.cluster.slots[slot]; redisAssert(n != NULL);
            sdsfree(ovrd_key);
        } else if (!keyindex) { return server.cluster.myself;
        } else // do for() loop
#else
