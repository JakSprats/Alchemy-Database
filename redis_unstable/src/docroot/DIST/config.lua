...
function ComputeCluster() -- NOTE: also used in promoteSelfToMaster()
  for inid, data in pairs(NodeData) do -- DEFAULTS
    if (data['synced']   == nil) then data['synced']   = false;            end
  ...
  for inid, data in pairs(NodeData) do
    if (data['active'] and data['slave'] == false and data['isb'] == false) then
  ...
  BridgeData = {};
  for inid, data in pairs(NodeData) do
    if (data['active'] and data['slave'] == false and data['isb']) then
  ...
  PeerData   =  {};
  for inid, data in pairs(NodeData) do
    if (data['active'] and data['slave'] == false and
        tonumber(data['bridge']) == BridgeId) then
  ...
  MasterData  = {};
  for inid, data in pairs(NodeData) do
    if (data['active'] and data['slave']) then
      MasterData[inid] = tonumber(data['masterid']);--TODO multiple slaves
    end
  end

  GlobalReadData = {};
  for inid, data in pairs(NodeData) do
    if (data['active'] and data['isb'] == false) then
  ...
  if (AmBridge) then
    NumPeers     = #PeerData + #BridgeData - 2; -- HBs from ALL except self(2X)
